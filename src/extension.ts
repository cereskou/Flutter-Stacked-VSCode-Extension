...
export function activate(context: vscode.ExtensionContext) {
  let regenerateDisposable = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      VsCodeActions.showInformationMessage(
        "Regenerating router and router constants"
      );
  ...
  let viewDisposable = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      let typeOfRoute = await inputTypeOfRoute();
      if (typeOfRoute === undefined) {
        typeOfRoute = TYPE_OF_ROUTE.MaterialPageRoute;
      }
  ...
  async function inputTypeOfRoute(): Promise<TYPE_OF_ROUTE | undefined> {
    let routeType = await VsCodeActions.getInputDropdown(Utils.TYPE_OF_ROUTE);

    return Utils.convertRouteToEnum(routeType);
  }

  async function inputTypeOfViewModel(): Promise<
    TYPE_OF_VIEWMODEL | undefined
  > {
    let viewModelType = await VsCodeActions.getInputDropdown(
      Utils.TYPE_OF_VIEWMODEL
    );
