...
export class RouterJSON {
  ...
  public async addRoute() {
    ...
    const file_data = FileSystemManager.readFileAsString(
      json_path,
      "router.json"
    );
    ...
    routes.push({
      ...
      transition: this.pageRoute ?? "MaterialPageRoute",
    });
  ...
  private get pathValue(): string {
    if (this.folders === undefined) {
      return path.join("views", this.snakeCasedFileName);
    }
